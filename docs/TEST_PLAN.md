...
| TC-73 | P1 | Public server tarball | Download hearth-server-<ver>.tar.gz from hearth-releases (logged out); unzip; run install.sh on a clean Linux box | Deps compile, service starts, claim code prints; server reachable — all with no private-repo access |
| TC-74 | P1 | Windows/Docker server installers | Run install-windows.ps1 on Windows; `docker compose up -d` on a Docker host | Each starts the server and yields a claim code; no source-repo access needed |
| TC-75 | P1 | Fuses + asar lock | Try to run the packaged app with ELECTRON_RUN_AS_NODE=1 and --inspect; try editing a file inside the asar | All refused/ignored; app runs only from the intact asar; no debugger attaches |
| TC-76 | P0 | AppImage self-update sticks | Install via install-linux.sh, let an update apply, then relaunch FROM THE MENU | Menu launches the NEW version; no repeat update prompt. (Regression: copy-over-running-AppImage throws ETXTBSY, leaving the on-disk file stale forever) |
| TC-77 | P1 | Launch version check | Run an older build with a newer release published | ~4s after launch a dialog offers Update now / Not now, warns an old app might not join the host; 'Update now' installs+restarts; 'Not now' dismisses and the app works |
| TC-78 | P1 | Installer cleanup | Run install-linux.sh with stale Hearth-<ver>.AppImage files and an old .desktop present | Stale AppImages and menu entries removed; single stable ~/Applications/Hearth.AppImage; ~/.config/Hearth (identity/settings) untouched |
