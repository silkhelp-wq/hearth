...
| FR-26 | Messages SHALL render safe markdown: fenced/inline code, bold/italic/strike, quotes, autolinked URLs — HTML SHALL always be escaped. A message that is a single bare https URL to an image (gif/png/jpg/webp/avif), a video file (mp4/webm/mov), or a YouTube video SHALL render as an inline embed (image, video player with metadata-only preload, or click-to-play thumbnail respectively); embeds are DOM-constructed, never innerHTML. |
